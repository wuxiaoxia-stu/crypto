...
#ifdef BUILD_AS_STATIC_LIBRARY
#include "NETCA_PKI_Device.h"
#endif
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoInit(void);
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoEnd(void);
...
/*
 *	NetcaPKICryptoGetPhysicalDeviceCount
 *		获取设备集中实际的物理设备的个数
 *	参数：
 *		hDeviceSet：设备集句柄
 *	返回值：
 *		成功返回设备集中实际的物理设备的个数，失败返回-1
 *
 */
int NETCADLLAPI NetcaPKICryptoGetPhysicalDeviceCount(NETCA_PKI_DEVICE_HANDLE_SET hDeviceSet);
...
/*
 *	NetcaPKICryptoAddDeviceProvider 
 *		增加一个设备DLL实现
 *	参数：
 *		dirName：输入参数，设备DLL将要保存的路径,UTF-8编码。如果dllContent为NULL且dllContentLength为0，则为设备DLL文件路径UTF-8编码。
 *		type：输入参数，设备的类型
 *		dllContent：输入参数，设备DLL文件的内容
 *		dllContentLength：输入参数，设备DLL文件的长度
 *		signature：输入参数，设备DLL的签名
 *		signatureLength：输入参数，设备DLL的签名长度
 *		param：输入参数，设备DLL的其他相关参数，JSON字符串，UTF-8编码
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoAddDeviceProvider(const char *dirName,int type,const unsigned char *dllContent,int dllContentLength,const unsigned char *signature,int signatureLength,const char* param);

#ifdef BUILD_AS_STATIC_LIBRARY
/*
 *	NetcaPKICryptoAddStaticDeviceProvider 
 *		增加一个设备静态实现
 *	参数：
 *		getDeviceFunctionList：输入参数，静态实现的函数列表。
 *		type：输入参数，设备的类型
 *		param：输入参数，设备的其他相关参数，JSON字符串，UTF-8编码
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoAddStaticDeviceProvider(DeviceGetFunctionList_Ptr getDeviceFunctionList,int type,const char* param);
#endif
/*
 *	NetcaPKICryptoDeleteDeviceProvider 
 *		删除一个用NetcaPKICryptoAddDeviceProvider注册的设备DLL实现
 *	参数：
 *		type：输入参数，设备的类型
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，不存在返回NETCA_PKI_NOTEXISIT
 *
 */
int NETCADLLAPI NetcaPKICryptoDeleteDeviceProvider(int type);
...
/*
 *	NetcaPKICryptoDeviceIsSupportPwdAuth
 *		判断加密设备是否使用密码的方式认证
 *	参数：
 *		hDevice：设备句柄
 *	返回值：
 *		如果是使用密码的方式认证返回1，否则返回0
 *
 */
int NETCADLLAPI NetcaPKICryptoDeviceIsSupportPwdAuth(NETCA_PKI_DEVICE_HANDLE hDevice);

/*
 *	NetcaPKICryptoDeviceGetState
 *		获取设备的状态，看看是否已经格式化过。
 *	参数：
 *		hDevice：设备句柄
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoDeviceGetState(NETCA_PKI_DEVICE_HANDLE hDevice,int *pState);

/*
 *	NetcaPKICryptoDeviceFormatDevice
 *		格式化设备。
 *	参数：
 *		hDevice：设备句柄
 ...
 */
int NETCADLLAPI NetcaPKICryptoDeviceFormatDevice(NETCA_PKI_DEVICE_HANDLE hDevice,const char *soPwd,int soPwdRetryNumber,const char *userPwd,int userPwdRetryNumber);

/*
 *	NetcaPKICryptoDeviceNewDevice
 *		根据subDeviceName名称，格式化出一个新设备。
 *	参数：
 *		hDevice：设备句柄
 *		flag：输入参数，设备标志
 ...
 */
int NETCADLLAPI NetcaPKICryptoDeviceNewDevice(NETCA_PKI_DEVICE_HANDLE hDevice,int flag, const char *subDeviceName, const char *soPwd, int soPwdRetryNumber, const char *userPwd, int userPwdRetryNumber, NETCA_PKI_DEVICE_HANDLE *phDevice2);


/*
 *	NetcaPKICryptoDeviceDeleteDevice
 *		删除逻辑设备。
 *	参数：
 *		hDevice：设备句柄
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，没有权限返回NETCA_PKI_ACCESSDENIED，不支持返回NETCA_PKI_UNSUPPORTED，失败返回NETCA_PKI_FAIL
 */
int NETCADLLAPI NetcaPKICryptoDeviceDeleteDevice(NETCA_PKI_DEVICE_HANDLE hDevice);
...
/*
 *	NetcaPKICryptoReVerifyUserPwd
 *		使用缓存的用户密码进行验证，如果没有缓存的密码又允许UI，则弹出对话框输入密码再验证
 *	参数：
 *		hDevice：设备句柄
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoReVerifyUserPwd(NETCA_PKI_DEVICE_HANDLE hDevice,const char *msg);
 ...
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，如果持久数据已经存在，返回NETCA_PKI_EXISIT，如果没权限创建持久数据，返回NETCA_PKI_ACCESSDENIED。如果不支持指定标识的持久数据，返回NETCA_PKI_UNSUPPORTED
 *
 */
 ...
 *		dataId：输入参数，持久数据标识
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，如果没有指定的持久数据，返回NETCA_PKI_NOTEXISIT，如果没权限删除持久数据，返回NETCA_PKI_ACCESSDENIED
 *
 */
int NETCADLLAPI NetcaPKICryptoDeleteData(NETCA_PKI_DEVICE_HANDLE hDevice,int dataId);

/*
 *	NetcaPKICryptoSetDeviceExtraInfo
 *		设置设备句柄相关的信息
 *	参数：
 *		hDevice：设备句柄
 *		data：输入参数，设备句柄相关的信息，为NULL则为清除以前相关的信息
 *		freeFunc：输入参数，释放设备句柄相关的信息的函数
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值。
 *
 */
int NETCADLLAPI NetcaPKICryptoSetDeviceExtraInfo(NETCA_PKI_DEVICE_HANDLE hDevice,void *data,NetcaPKICryptoFreeDeviceExtraDataFunction_Ptr freeFunc);

/*
 *	NetcaPKICryptoGetDeviceExtraInfo
 *		获取设备句柄相关的信息
 *	参数：
 *		hDevice：设备句柄
 *	返回值：
 ...
 *
 */
void* NETCADLLAPI NetcaPKICryptoGetDeviceExtraInfo(NETCA_PKI_DEVICE_HANDLE hDevice);
...
/*
 *	NetcaPKICryptoGetKeypairFromString
 *		根据密钥对的字符串信息获取密钥对句柄
 *	参数：
 *		str：输入参数，密钥对的字符串信息
 *		flag:输入参数，设备的标志
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoGetKeypairFromString(const char *str,int flag,const char *pwd,NETCA_PKI_KEYPAIR_HANDLE *phKeypair);
 ...
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，不支持的算法返回NETCA_PKI_UNSUPPORTED，如果没权限产生密钥对，返回NETCA_PKI_ACCESSDENIED
 *
 */
 ...
 *		phKeypair：输出参数，成功返回产生的密钥对句柄，失败返回NULL
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，不支持返回NETCA_PKI_UNSUPPORTED，如果没权限导入密钥对，返回NETCA_PKI_ACCESSDENIED
 *
 */
 ...
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，不支持的公钥返回NETCA_PKI_UNSUPPORTED。
 *
 */
 ...
 *		index：输入参数，索引号，从0算起
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，如果没有权限删除证书，返回NETCA_PKI_ACCESSDENIED
 *
 */
 ...
 *		cert：证书
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL，如果没有权限写证书，返回NETCA_PKI_ACCESSDENIED
 *
 */
...
/*
 *	NetcaPKICryptoIsSupportedSignAlgorithm
 *		询问设备是否支持某个签名算法。
 *	参数：
 *		hDevice：设备句柄
 *		algo：输入参数，签名算法
 *		param：输入参数，签名算法相关的参数
 *		
 *	返回值：
 *		如果支持返回1，不支持返回0。
 */
int NETCADLLAPI NetcaPKICryptoIsSupportedSignAlgorithm(NETCA_PKI_DEVICE_HANDLE hDevice,int algo,const void *param);
 ...
 *		data：输入参数，要进行Hash的数据
 *		dataLen：输入参数，要进行Hash的数据长度
 *		hashValue：输出参数，成功返回Hash值
 *		hashValueLen：输出参数，成功返回Hash值长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoHash(int hashAlgo,const unsigned char* data,int dataLen,unsigned char** hashValue,int* hashValueLen);
 ...
 *		out：输出参数，返回的数据
 *		outLen:输入参数，out的字节数
 *		pRetOutLen：输出参数，返回输出长度。如果out为NULL，则返回可以容纳结果的长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回NETCA_PKI_FAIL
 *
 */
int NETCADLLAPI NetcaPKICryptoCipher(NETCA_PKI_CIPHER_HANDLE hCipher,const unsigned char *in,long long inLen,unsigned char *out,long long outLen,long long *pRetOutLen);
...
/*
 *	NetcaPKICryptoIsDevicePullOut
 *		判断设备插拔记录里，是否存在t时间后的拔出记录。
 *	参数：
 *		type：输入参数，设备类型
 *		sn：输入参数，设备的序列号
 *		t:输入参数，时间
 *	返回值：
 *		存在记录返回1，不存在返回0，获取记录信息失败返回-1
 *
 */
int NETCADLLAPI NetcaPKICryptoIsDevicePullOut(int type,const char *sn,const char *t);
...
/*
 *	NetcaPKICryptoClearPullOutInfo
 *		清除设备插拔信息。
 *	参数：
 *		无
 *	返回值：
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoClearPullOutInfo(void);

/*
 *	NetcaPKICryptoNotifyPullOutEvent
 *		通知设备插拔事件。
 *	参数：
 *		无
 *	返回值：
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoNotifyPullOutEvent();

/*
 *	NetcaPKICryptoRegisterPullOutCallback
 *		注册设备插拔回调函数。
 *	参数：
 *		callback：输入参数，回调函数
 *		context：参数，回调函数的参数
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoRegisterPullOutCallback(NetcaDevicePullOutCallback_Ptr callback,void *context);

/*
 *	NetcaPKICryptoUnregisterPullOutCallback
 *		反注册设备插拔回调函数。
 *	参数：
 *		callback：输入参数，回调函数
 *		context：参数，回调函数的参数
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoUnregisterPullOutCallback(NetcaDevicePullOutCallback_Ptr callback,void *context);

/*
 *	NetcaPKICryptoMonitorDevice
 *		监控设备
 *	参数：
 *		callback：输入参数，回调函数
 *		context：参数，回调函数的参数
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoMonitorDevice(NetcaMonitorDeviceCallback_Ptr callback,void *context);
#endif
...
#else
int NETCADLLAPI NetcaPKICryptoSetWindowHandle(NETCA_PKI_DEVICE_HANDLE hDevice,void* hwnd);
#endif
/*
...
/*
 *	NetcaPKICryptoSetEvent
 *		广播事件通知。
 *	参数：
 *		eventType：输入参数，事件类型
 *		deviceType：输入参数，设备类型
 *		deviceType：输入参数，设备序列号
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoSetEvent(int eventType,int deviceType,const char *deviceSn);

/*
 *	NetcaPKICryptoWait
 *		等待设备事件通知，发生事件后，则返回所有未获取的事件的类型，设备类型和设备序列号。如果在事件发生之前调用则马上返回。
 *	参数：
 *		eventType：输出参数，返回的事件类型
 *		deviceType：输出参数，返回的设备类型
 *		deviceType：输出参数，返回的设备序列号
 *		deviceType：输出参数，返回的事件个数
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoWait(int **eventType,int** deviceType,char ***deviceSn,int* count);
...
/*
 *	NetcaPKICryptoSetKeyagreementMsg
 *		设置密钥协商的提示信息。
 *	参数：
 *		hDevice：设备句柄
 *		flag：输入参数，保留参数，当前只能取0
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoSetKeyagreementMsg(NETCA_PKI_DEVICE_HANDLE hDevice,int flag,const char *msg);
 ...
 *
 */
int NETCADLLAPI NetcaPKICryptoBase64(int enc,int flag,const unsigned char * in,int inlen,unsigned char **out,int *poutlen);
...
/*
 *	NetcaPKICryptoHexEncode
 *		Hex编码
 *	参数：
 *		cap：输入参数，为真表示大写，为假表示小写
 *		data：输入参数，要加密的数据
 *		dataLen：输入参数，要加密的数据长度
 *		hex：输出参数，返回编码后的数据
 *		hexLen：输出参数，返回hex的长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoHexEncode(int cap,const unsigned char* data,int dataLen,char** hex,int* hexLen);

/*
 *	NetcaPKICryptoHexDecode
 *		Hex解码
 *	参数：
 *		hex：输入参数，Hex格式的数据
 *		hexLen：输入参数，Hex格式的数据长度
 *		data：输出参数，成功返回解码后的数据
 *		dataLen：输出参数，成功返回解码后的数据长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoHexDecode(const char* hex,int hexLen,unsigned char** data,int* dataLen);
...
/*
 *	NetcaPKICryptoKeyPairToPKCS8PrivateKeyInfo
 *		内部格式的密钥对编码转换为PKCS#8的PrivateKeyInfo的DER编码
 *	参数：
 *		flag：输入参数，标志
 *		in：输入参数，内部格式的密钥对编码
 *		inLen：输入参数，内部格式的密钥对编码的字节数
 *		attrsEncode：输入参数，属性的DER编码，可以为NULL
 *		attrsEncodeLen：输入参数，RSA的公钥指数
 *		out：输出参数，PKCS#8的PrivateKeyInfo的DER编码
 *		outLen：输出参数，PKCS#8的PrivateKeyInfo的DER编码的字节数
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoKeyPairToPKCS8PrivateKeyInfo(int flag,const unsigned char *in,int inLen,const unsigned char *attrsEncode,int attrsEncodeLen,unsigned char **out,int *outLen);

/*
 *	NetcaPKICryptoPKCS8PrivateKeyInfoToKeyPair
 *		PKCS#8的PrivateKeyInfo的DER编码的私钥转换为内部格式的密钥对编码
 *	参数：
 *		flag：输入参数，标志
 *		in：输入参数，PKCS#8的PrivateKeyInfo的DER编码
 *		inLen：输入参数，PKCS#8的PrivateKeyInfo的DER编码的字节数
 *		out：输出参数，内部格式的密钥对编码
 *		outLen：输出参数，内部格式的密钥对编码的字节数
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoPKCS8PrivateKeyInfoToKeyPair(int flag,const unsigned char *in,int inLen,unsigned char **out,int *outLen);


/*
 *	NetcaPKICryptoEncryptPKCS8PrivateKeyInfo
 *		加密PrivateKeyInfo的DER编码得到EncryptedPrivateKeyInfo的DER编码
 *	参数：
 *		kdfAlgo：输入参数，KDF算法
 *		param：输入参数，KDF参数
 *		pwd：输入参数，口令
 ...
 *		in：输入参数，PKCS#8的PrivateKeyInfo的DER编码
 *		inLen：输入参数，PKCS#8的PrivateKeyInfo的DER编码的字节数
 *		out：输出参数，EncryptedPrivateKeyInfo的DER编码
 *		outLen：输出参数，EncryptedPrivateKeyInfo的DER编码的字节数
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoEncryptPKCS8PrivateKeyInfo(int kdfAlgo,const void *param,const unsigned char *pwd,int pwdLen,int encAlgo,const unsigned char *in,int inLen,unsigned char **out,int *outLen);


/*
 *	NetcaPKICryptoDecryptPKCS8PrivateKeyInfo
 *		解码EncryptedPrivateKeyInfo的DER编码得到PrivateKeyInfo的DER编码
 *	参数：
 *		flag：输入参数，标识，通常为0
 *		param：输入参数，KDF参数
 *		pwd：输入参数，口令
 *		pwdLen：输入参数，口令长度
 *		in：输入参数，EncryptedPrivateKeyInfo的DER编码
 *		inLen：输入参数，EncryptedPrivateKeyInfo的DER编码的字节数
 *		out：输出参数，PrivateKeyInfo的DER编码
 *		outLen：输出参数，PrivateKeyInfo的DER编码的字节数
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoDecryptPKCS8PrivateKeyInfo(int flag,const unsigned char *pwd,int pwdLen,const unsigned char *in,int inLen,unsigned char **out,int *outLen);
...
/*
 *	NetcaPKICertMatchHost
 *		判断证书是否匹配主机名或者IP地址，用于验证TLS服务器证书
 *	参数：
 *		cert：证书句柄
 *		host：输入参数，主机名或者IP地址，UTF-8编码
 *	返回值：
 *		如果匹配返回1，不匹配返回0，如果是通配符匹配返回-1。
 *
 */
int NETCADLLAPI NetcaPKICertMatchHost(NETCA_PKI_CERTIFICATE_HANDLE cert,const char *host);

/*
 *	NetcaPKICertMatchPublicKey
 *		判断证书是否和公钥匹配
 *	参数：
 *		cert：证书句柄
 *		hPublicKey：公钥句柄
 ...
 *
 */
int NETCADLLAPI NetcaPKICertMatchPublicKey(NETCA_PKI_CERTIFICATE_HANDLE cert,NETCA_PKI_PUBLICKEY_HANDLE hPublicKey);
...
/*
 *	NetcaPKICertIsMultiValueStringAttribute
 *		判断证书的字符串类型的属性是否多值属性
 *	参数：
 *		attrId：输入参数，属性
 *	返回值：
 ...
 *
 */
int NETCADLLAPI NetcaPKICertIsMultiValueStringAttribute(int attrId);

/*
 *	NetcaPKICertGetAttributeType
 *		判断证书的属性的类型
 *	参数：
 *		attrId：输入参数，属性
 *	返回值：
 ...
 *
 */
int NETCADLLAPI NetcaPKICertGetAttributeType(int attrId);
 ...
 *
 */
int NETCADLLAPI NetcaPKICertFindExtension(NETCA_PKI_CERTIFICATE_HANDLE cert,const char *oid,int *pcritical,unsigned char **pvalue,int *pvaluelen);
...
/*
 *	NetcaPKICertExportPFXEx
 *		导出PFX
 *	参数：
 *		certs：要导出PFX的证书
 *		count：输入参数，证书的个数
 *		pwd：输入参数，PFX的密码，UTF-8编码
 *		param：输入参数，相关的参数，可以为NULL
 *		pfx：输出参数，导出的PFX编码
 *		pfxLen：输出参数，pfx的长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICertExportPFXEx(NETCA_PKI_CERTIFICATE_HANDLE certs[],int count,const char *pwd,NETCA_PKI_PFX_EXPORT_PARAM param,unsigned char **pfx,int *pfxLen);
...
/*
 *	NetcaPKICertExportP7c
 *		把证书库里的所有证书，导出成P7c
 *	参数：
 *		store：要导出证书库
 *		p7c：输出参数，导出的P7c的DER编码
 *		p7cLen：输出参数，P7c的长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICertExportP7c(NETCA_PKI_CERT_STORE store,unsigned char **p7c,int *p7cLen);
...
/*
 *	NetcaPKICertCRLVerifyCertByCRLObject
 *		使用CRL验证证书的作废状态
 *	参数：
 *		cert：证书
 *		crlObj：输入参数，CRL句柄
 ...
 *
 */
int NETCADLLAPI NetcaPKICertCRLVerifyCertByCRLObject(NETCA_PKI_CERTIFICATE_HANDLE cert,NETCA_PKI_CRL_HANDLE crlObj,const char *checktime,int *status,NETCA_PKI_REVOKEDCERT_HANDLE *info);
...
/*
 *	NetcaPKICertVerifyAddCRLObject
 *		添加CRL
 *	参数：
 *		hCertVerify：证书验证句柄
 *		crlObj:输入参数，CRL对象
 ...
 *
 */
int NETCADLLAPI NetcaPKICertVerifyAddCRLObject(NETCA_PKI_CERT_VERIFY_HANDLE hCertVerify,NETCA_PKI_CRL_HANDLE crlObj);
...
/*
 *	NetcaPKICertVerifyAddOCSPVACert
 *		添加配置的OCSP证书，必须在NetcaPKICertVerifyAddOCSP之前调用
 *	参数：
 *		hCertVerify：证书验证句柄
 *		issuerCert：颁发者证书
 *		vaCert:输入参数，OCSP证书
 ...
 *
 */
int NETCADLLAPI NetcaPKICertVerifyAddOCSPVACert(NETCA_PKI_CERT_VERIFY_HANDLE hCertVerify,NETCA_PKI_CERTIFICATE_HANDLE issuerCert,NETCA_PKI_CERTIFICATE_HANDLE vaCert);
...
/*
 *	NetcaPKICertVerifyGetRevokeInfoSource
 *		获取相关的作废信息的来源，只对正常和作废有效
 *	参数：
 *		hCertVerify：证书验证句柄
 *		index：输入参数，证书路径中的证书索引
 *		source：输出参数，返回的作废信息来源数组
 *		count：输出参数，返回的作废信息来源个数
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKICertVerifyGetRevokeInfoSource(NETCA_PKI_CERT_VERIFY_HANDLE hCertVerify, int index,NETCA_PKI_REVOKE_INFO_SOURCE_HANDLE **source,int *count);

/*
 *	NetcaPKICertVerifyFreeRevokeInfoSource
 *		释放作废信息的来源
 *	参数：
 *		source：作废信息来源数组
 *		count：作废信息来源个数
 ...
 *
 */
int NETCADLLAPI NetcaPKICertVerifyFreeRevokeInfoSource(NETCA_PKI_REVOKE_INFO_SOURCE_HANDLE *source,int count);
...
/*
 *	NetcaPKIGetDerEncodeLength
 *		根据部分数据获取DER编码的长度
 *	参数：
 *		buf：输入参数，DER编码的部分数据
 *		bufLen：输入参数，buf的长度
 *		pLength：输出参数，成功返回DER编码的长度，如果是BER编码的不定长度，返回-1
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKIGetDerEncodeLength(const unsigned char *buf,int bufLen,int *pLength);

/*
 *	NetcaPKIEncodeSetOf
 *		根据编码SET OF
 *	参数：
 *		items：输入参数，DER编码的数据项
 *		itemLens：输入参数，bDER编码的数据项长度
 *		count:输入参数，数据项数目
 *		setOf：输出参数，返回SET OF的编码
 *		pSetOfLen：输出参数，返回的SET OF的编码长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKIEncodeSetOf(const unsigned char *items[],int itemLens[],int count,unsigned char **setOf,int* pSetOfLen);

/*
 *	NetcaPKISetLastError
 *		设置错误号
 *	参数：
 *		errorNo：输入参数，错误号
 *	返回值：
 ...
 *
 */
int NETCADLLAPI NetcaPKISetLastError(int errorNo);

/*
 *	NetcaPKIGetLastError
 *		获取最后一个错误号
 *	参数：
 *		无
 *	返回值：
 ...
 *
 */
int NETCADLLAPI NetcaPKIGetLastError(void);
/*
 *	NetcaPKIGetErrorMsg
 *		获取错误号对应的错误信息
 *	参数：
 *		errorCode：输入参数，错误号
 *	返回值：
 *		成功返回对应的错误信息UTF-8编码，失败返回NULL
 *
 */
char* NETCADLLAPI NetcaPKIGetErrorMsg(int errorCode);

/*
 *	NetcaPKIJson2Der
 *		根据JSON转换为DER编码
 *	参数：
 *		json：输入参数，JSON的编码，UTF-8
 *		jsonLen：输入参数，JSON的编码长度
 *		der：输出参数，成功返回DER编码
 *		derLen：输出参数，成功返回的DER编码编码长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKIJson2Der(const unsigned char *json,int jsonLen,unsigned char **der,int* derLen);
 ...
 *	参数：
 *		der：输入参数，DER编码
 *		derLen：输入参数，DER编码编码长度
 *		json：输出参数，成功返回JSON的编码，UTF-8
 *		jsonLen：输出参数，成功返回JSON的编码长度
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI NetcaPKIDer2Json(const unsigned char *der,int derLen,unsigned char **json,int *jsonLen);

/*
 *	NetcaPKIRandomPokerTest
 *		扑克测试
 *	参数：
 *		byteString：输入参数，输入的随机数
 *		byteLength：输入参数，随机数长度
 *		m：输入参数，m值
 *		alpha：输入参数，alpha值
 *	返回值：
 *		通过测试返回NETCA_PKI_SUCCESS，否则返回其他值
 *
 */
int NETCADLLAPI NetcaPKIRandomPokerTest(const unsigned char *byteString,int byteLength,int m,double alpha);

/*
 *	NetcaPKICryptoSCVPDPD
 *		使用SCVP协议获取证书路径
 *	参数：
 *		cert:输入参数，要构造证书路径的证书
 *		url：输入参数，SCVP的服务器URL
 *		wantBackType：输入参数，返回的数据类型
 *		responseFlags：输入参数，响应的标记
 *		certpath：输出参数，成功返回相关的证书路径里的所有证书
 *		certpathCount：输出参数，成功返回相关的证书路径里证书个数
 *		tas：输出参数，成功返回相关的根证书
 *		taCount：输出参数，成功返回相关的根证书个数
 *		revokeSource：输出参数，成功返回相关的作废信息
 *		revokeSourceCount：输出参数，成功返回相关的作废信息个数
 *	返回值：
 *		通过测试返回NETCA_PKI_SUCCESS，否则返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoSCVPDPD(NETCA_PKI_CERTIFICATE_HANDLE cert,const char *url,int wantBackType,int responseFlags,NETCA_PKI_CERTIFICATE_HANDLE **certpath,int* certpathCount,NETCA_PKI_CERTIFICATE_HANDLE **tas,int* taCount,NETCA_PKI_REVOKE_INFO_SOURCE_HANDLE **revokeSource,int *revokeSourceCount);

/*
 *	NetcaPKICryptoSCVPDPV
 *		使用SCVP协议验证证书
 *	参数：
 *		cert:输入参数，要验证的证书
 *		url：输入参数，SCVP的服务器URL
 *		verifyOption：输入参数，验证选项，可以为NULL，否则为JSON对象的编码
 *		verifyRevoke：输入参数，是否包括验证作废状态
 *		responseFlags：输入参数，响应的标记
 *		scvpCert：输入参数，SCVP的证书
 *		status：输出参数，成功返回证书的状态
 *	返回值：
 *		通过测试返回NETCA_PKI_SUCCESS，否则返回其他值
 *
 */
int NETCADLLAPI NetcaPKICryptoSCVPDPV(NETCA_PKI_CERTIFICATE_HANDLE cert,const char *url,const char *verifyOption,int verifyRevoke,int responseFlags,NETCA_PKI_CERTIFICATE_HANDLE scvpCert,int* status);
