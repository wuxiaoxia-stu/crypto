...
/*
 *	DeviceInit
 *		初始化DLL使用的全局资源，在使用设备接口前必须先调用该函数或DeviceInitEx
 *	参数：
 *		无
 *	返回值：
...
/*
 *	DeviceInitEx
 *		初始化DLL使用的全局资源，在使用设备接口前必须先调用该函数或DeviceInit
 *	参数：
 *		initString：输入参数，初始化字符串,UTF-8编码。如果为NULL，则相对于DeviceInit
 *	返回值：
 *		成功返回NETCA_PKI_SUCCESS，失败返回其他值
 *
 */
int NETCADLLAPI DeviceInitEx(const char* initString);
...
typedef int (NETCADLLAPI *DeviceInitEx_Ptr)(const char* initString);

#define NETCA_PKI_DEVICE_FUNCTION_CURRENT_VERSION		2
...
struct NETCA_PKI_Device_FunctionList_st
{
	...
	DeviceInitEx_Ptr m_DeviceInitEx; /* 版本2 新增*/
};
