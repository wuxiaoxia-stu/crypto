 ...
 *			成功返回条件变量，失败返回NULL
 */
NETCA_Condition_Ptr NETCAAPI netca_condition_new(void);

/*
 *		netca_condition_wait
 *			等待一个条件，必须在上锁的情况下等待，而且要在循环中等待
 *		参数：
 *			cond：条件变量
 *			lock：锁
 *		返回值：
 *			成功返回NETCA_OK，失败返回其他值
 */
int NETCAAPI netca_condition_wait(NETCA_Condition_Ptr cond,NETCA_Lock lock);

/*
 *		netca_condition_signal
 *			发信号给一个等待条件的线程
 *		参数：
 *			cond：条件变量
 *		返回值：
 *			成功返回NETCA_OK，失败返回其他值
 */
int NETCAAPI netca_condition_signal(NETCA_Condition_Ptr cond);

/*
 *		netca_condition_broadcast
 *			广播给所有等待条件的线程
 *		参数：
 *			cond：条件变量
 *		返回值：
 *			成功返回NETCA_OK，失败返回其他值
 */
int NETCAAPI netca_condition_broadcast(NETCA_Condition_Ptr cond);

/*
 *		netca_condition_free
 *			释放条件变量
 *		参数：
 *			cond：条件变量
 *		返回值：
 *			无
 */
void NETCAAPI netca_condition_free(NETCA_Condition_Ptr cond);
