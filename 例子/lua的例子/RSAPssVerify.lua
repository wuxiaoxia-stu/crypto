...
function Usage(filename)
    print(string.format("Usage: lua %s cert hashAlgo mgfAlgo saltLen inFileName sigantureFileName",filename))
    ...
    print(ToANSIString("\tsigantureFileName：签名文件名"))
    print(ToANSIString("说明"))
    print(ToANSIString("\t验证RSA PSS签名"))
end
...
local publicKey=assert(certObj:PublicKey(0x10000002))
certObj:Free()
...
local fOut=io.open(outFileName,"rb")
local signature=fOut:read("*a")
fOut:close()

local result=verifyObj:Final(signature)
verifyObj:Free()
if result then
