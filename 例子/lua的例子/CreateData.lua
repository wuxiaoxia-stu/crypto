...
local mode=CryptCore.HexDecode(arg[4])
if mode==nil then
    print(toANSIString("访问权限不是Hex编码"))
    os.exit(-1)
end
...
local rv=deviceObj:CreateData(id,mode,length)
deviceObj:Free()

if rv==1 then
    print(toANSIString("创建持久数据成功"))
else
    print(toANSIString(string.format("创建持久数据失败，错误号为:%d",rv)))
end
