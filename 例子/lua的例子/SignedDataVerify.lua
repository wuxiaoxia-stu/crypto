...
function Usage(filename)
    ...
    print(ToANSIString("\ttbsFileName：要写入的原文文件名"))
    print(ToANSIString("\tsignatureFileName：要验证的签名文件名"))
    print(ToANSIString("说明"))
    print(ToANSIString("\t对带原文的signatureFileName文件进行验证，得到的原文保存到tbsFileName文件中"))
end
...
fOut=io.open(tbsFileName,"wb")
if fOut==nil then
    sign:Free()
    fIn:close()
    print(ToANSIString(string.format("打开输出文件(%s)失败",CryptCore.ACPToUTF8(tbsFileName))));
    os.exit(-1)
end
