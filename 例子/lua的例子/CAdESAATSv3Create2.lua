...
local hashValue=hashFile(tsaHashAlgo,tbsFileName)

if not sign:AddArchiveTimeStampV3ByHashValue(0,tsaHashAlgo,url,hashValue) then
    sign:Free()
...
local timeStamp
if tsaCert==nil then
    timeStamp=assert(sign:VerifyArchiveTimeStampV3ByHashValue(0,count-1,hashValue))
else
    timeStamp=assert(sign:VerifyArchiveTimeStampV3ByHashValue(0,count-1,hashValue,tsaCert))
end
