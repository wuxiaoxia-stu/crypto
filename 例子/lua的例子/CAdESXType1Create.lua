...
function Usage(filename)
    ...
    print(ToANSIString("\tnewSignFileName：生成的CAdES-X Type1文件名"))
    ...
    print(ToANSIString("\t在不带原文的CAdES-C中添加时间戳，构成CAdES-X Type1"))
end
...
if not sign:AddEscTimeStamp(0,tsaHashAlgo,url) then
    sign:Free()
    print(ToANSIString("添加CAdES-C-time-stamp时间戳失败"));
    os.exit(-1)
end
outData=sign:Encode()
if not outData then
...
local timeStamp
if tsaCert==nil then
    timeStamp=assert(sign:VerifyEscTimeStamp(0,0))
else
    timeStamp=assert(sign:VerifyEscTimeStamp(0,0,tsaCert))
end
print(ToANSIString(string.format("CAdES-C-time-stamp时间:%s",timeStamp:GetTime())))
timeStamp:Free()
...
print(ToANSIString("构造CAdES-X Type1成功"));
