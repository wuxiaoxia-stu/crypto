...
function Usage(filename)
    ...
    print(ToANSIString("\tsignFileName：CAdES-A ATSv2文件名"))
    print(ToANSIString("\ttsaCertFileName：信任的时间戳证书文件名"))
    print(ToANSIString("说明"))
    print(ToANSIString("\t验证不带原文的CAdES-A ATSv2"))
end
...
local tbs=data
if not sign:DetachedVerify(tbs,signedData) then
    sign:Free()
...
count=sign:GetArchiveTimestampV2Count(0)
if count<1 then
    print(ToANSIString("不是CAdES-A ATSv2"))
    sign:Free()
    os.exit(-1)
end
...
for i=1,count do
    if tsaCert==nil then
        timeStamp=sign:VerifyArchiveTimeStampV2(0,i-1,signedData,tbs)
    else
        timeStamp=sign:VerifyArchiveTimeStampV2(0,i-1,signedData,tbs,tsaCert)
    end
    if timeStamp==nil then
        print(ToANSIString("验证CAdES-A ATSv2失败"))
        sign:Free()
        os.exit(-1)
    end
    print(ToANSIString(string.format("第%d个archiveTimestampV2时间:%s",i,timeStamp:GetTime())))
    timeStamp:Free()
end
print(ToANSIString("验证CAdES-A ATSv2成功"))
sign:Free()
