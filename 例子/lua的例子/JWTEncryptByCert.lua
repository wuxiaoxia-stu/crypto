...
function Usage(filename)
    print(string.format("Usage: lua %s [-nest] cekAlgo kekAlgo cert protectedheaderFileName outFileName innerJWTFileName|claimName1 claimValue1 ... claimNameN claimValueN",filename))
    ...
    print(ToANSIString("\t进行JWT的加密，结果保存到outFileName文件中"))
end
...
local certIdType=4
local jwtValue=jwtObj:EncryptByCert(cekAlgo,kekAlgo,certObj,certIdType)
certObj:Free()
jwtObj:Free()
