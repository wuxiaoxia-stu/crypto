...
function Usage(scriptName)
    ...
    print(ToANSIString("\t对inFileName文件使用指定的设备进行SSF33 CTR模式的加密或者解密运算，结果保存到outFileName文件中"))
end
...
if argCount==9 then
    ...
    if result==false then
        ...
        elseif retryNumer==-1 then
            print(ToANSIString("验证密码失败"))
        else
...
cipherObj=NewSsf33Ctr(deviceObj,enc,key,iv,countBits)
deviceObj:Free()
