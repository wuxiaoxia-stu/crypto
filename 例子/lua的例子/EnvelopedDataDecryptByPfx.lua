...
function Usage(filename)
    print(string.format("Usage: lua %s inFileName outFileName pfxFileName [pwd]",filename))
...
local pfxFileName=arg[3]
local pfx=GetFileConetent(pfxFileName)
if pfx==nil then
...
if not env:AddStore(pfxStore) then
    pfxStore:Close()
    env:Free()
    print(ToANSIString("添加PFX证书失败"));
    os.exit(-1)
end
pfxStore:Close()
