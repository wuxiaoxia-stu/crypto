...
function Usage(scriptName)
    ...
    print(ToANSIString("\tinFileName：输入文件名，其内容要进行加密或者解密运算"))
    print(ToANSIString("\toutFileName：输出文件名"))
    print(ToANSIString("说明"))
    print(ToANSIString("\t对inFileName文件进行3DES CBC模式的加密或者解密运算，结果保存到outFileName文件中"))    
end

function New3desCbc(enc,key,iv)
...
cipherObj=assert(New3desCbc(enc,key,iv))
