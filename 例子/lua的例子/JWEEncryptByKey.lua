...
function Usage(filename)
    print(string.format("Usage: lua %s [-compact] cekAlgo kekAlgo key protectedheaderFileName sharedUnprotectedheaderFileName recipientUnprotectedheaderFileName aadFileName clearFileName outFileName",filename))
...
if recipientUnprotectedheaderFileName~="" then
    ...
    if not jweObj:AddKey(kekAlgo,key,header) then
        jweObj:Free();
        print(ToANSIString("添加密钥失败"));
        os.exit(-1)
    end
else
    if not jweObj:AddKey(kekAlgo,key) then
    --if not jweObj:AddKey(kekAlgo,key,"",{salt="1234567890",iter=4096}) then
        jweObj:Free();
        print(ToANSIString("添加密钥失败"));
        os.exit(-1)
    end
