...
local result,retryNumer= deviceObj:UnlockPwd(pwdType,soPwd,newPwd)
deviceObj:Free()
if result==false then
    if retryNumer==0 then
        print(toANSIString("验证解锁密码失败，设备已经锁死"))
    else
        print(toANSIString(string.format("验证解锁密码失败，还剩%d次重试机会",retryNumer)))
    end
    os.exit(-1)
elseif result==nil then
    print(toANSIString("解锁密码出错"))
    os.exit(-1)
end
    
print(toANSIString("解锁密码成功"))