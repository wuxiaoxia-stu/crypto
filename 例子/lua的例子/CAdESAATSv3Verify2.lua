...
local hashValue
for i=1,count do
    hashAlgo=sign:GetArchiveTimestampV3HashAlgo(0,i-1)
    hashValue=assert(CryptCore.Hash(hashAlgo,data))
    if tsaCert==nil then
        timeStamp=sign:VerifyArchiveTimeStampV3ByHashValue(0,i-1,hashValue)
    else
        timeStamp=sign:VerifyArchiveTimeStampV3ByHashValue(0,i-1,hashValue,tsaCert)
    end
