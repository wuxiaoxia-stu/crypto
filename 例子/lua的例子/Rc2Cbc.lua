...
function Usage(scriptName)
    print(string.format("lua %s /e|/d key iv effectiveBits inFileName outFileName",scriptName))
    ...
    print(ToANSIString("\t对inFileName文件进行RC2 CBC模式的加密或者解密运算，结果保存到outFileName文件中"))
end

function NewRc2Cbc(enc,key,iv,bits)
...
cipherObj=assert(NewRc2Cbc(enc,key,iv,effectiveBits))
