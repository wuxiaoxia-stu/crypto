...
int main(int argc,char **argv)
{
	...
	try
	{
		...
		CCertificate cert=CCertificate::SelectCert("Decrypt","Algorithm='SM2' && CertType='Encrypt'");
		if(cert.isNull())
		{
		...
		keypair.decrypt(NETCA_PKI_ALGORITHM_SM2_ENC,buf,bufLen,out);
	...
	catch(CPKIException& e)
	{
		...
		printf("sm2 decrypt fail,error code£º%d£¬error msg£º%s\n",e.errorCode(),e.what());
		return -1;
	}
}
