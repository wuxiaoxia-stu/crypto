...
if arg_count!=7:
    print("Usage: python signeddataverifyaddocsp.py tbs_file_name sign_file_name issuer_cert_file_name root_cert_file_name ocspcert_file_name ocspres_data_file_name")
    sys.exit(-1)
...
with SignedData(False) as sign_obj:
    ...
    with Certificate(root_cert_data) as cert1: 
        sign_obj.add_rootcert(cert1)
    #添加CA证书,验证级别为2，3，4时会使用到
    with Certificate(issuer_cert_data) as cert2: 
        sign_obj.add_cert(cert2)
    ...
    sign_obj.set_int_attribute(54,1,0);
