...
namespace CreateSignedAndEnvelopedData2
{
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length != 4)
            {
                Console.WriteLine("Usage: CreateSignedAndEnvelopedData2 enCert signCert inFile outFile");
                return;
            }
            ...
            using (Certificate enCert = GetCert(enCertFileName))
            {
                ...
                using (Certificate signCert = GetCert(signCertFileName))
                {
                    ...
                    using (SignedAndEnvelopedData signedAndEnvelopedData = new SignedAndEnvelopedData(true))
                    {
                        ...
                        signedAndEnvelopedData.SetSignCallback(SignHashCallback);
