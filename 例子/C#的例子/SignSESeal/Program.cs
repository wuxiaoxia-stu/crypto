...
namespace SignSESeal
{
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length != 4)
            {
                Console.WriteLine("Usage: SignSESeal signCertFileName tbsFileName stampFileName sealFileName");
                return;
            }
            ...
            using (SEStamp stamp = new SEStamp(stampData))
            {
                ...
                using (SESeal seal = new SESeal(format))
                {
                    ...
                    using (Certificate signCert = GetCert(signCertFileName))
                    {
                        ...
                        if (keypair == null)
                        {
                            throw new Exception("签章证书没有私钥");
                        }
                        keypair.Dispose();

                        byte[] encode=seal.Sign(signAlgo, signCert);
