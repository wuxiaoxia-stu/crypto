...
namespace SignSEStamp2
{
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length != 4)
            {
                Console.WriteLine("Usage: SignSEStamp2 issuerCertFileName signAlgo templateFileName stampFileName");
                return;
            }
            ...
            using (Certificate issuerCert = GetCert(issuerCertFileName))
            {
                ...
                using (SEStamp stamp = new SEStamp(format))
                {
                    ...
                    byte[] encode = stamp.Sign(signAlgo, issuerCert,SignCallback);
