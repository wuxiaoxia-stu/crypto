...
namespace VerifyCertByOCSP
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Usage: VerifyCertByOCSP cert ocspUrl [ocspCert]");
                return;
            }
            using (Certificate cert = GetCert(args[0]))
            {
                ...
                if (args.Length == 2)
                {
                    rv = cert.VerifyRevokeWithOcsp(url, out info);
                }
                else
                {
                    using (Certificate ocspCert = GetCert(args[2]))
                    {
                        rv = cert.VerifyRevokeWithOcsp(url, ocspCert, out info);
                    }
                }
