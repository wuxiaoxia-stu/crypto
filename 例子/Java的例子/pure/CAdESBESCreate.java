...
public class CAdESBESCreate {

	public static void main(String[] args) throws Exception{
		int argCount=args.length;
		if(argCount!=7 && argCount!=8) {
            System.out.println("Usage: java CAdESBESCreate [-detached] signAlgo tbsFileName certFileName keystoreFileName pwd hashAlgo outputFileName");
            return;
        }
		int pos=0;
		...
		String keystoreFileName=args[pos];
		pos++;
		
		char[] pwd=args[pos].toCharArray();
		pos++;
		
		JCESigner signer=createSigner(cert,keystoreFileName,pwd);
	...
	private static JCESigner createSigner(X509Certificate cert,String keystoreFileName,char[] pwd) throws Exception{
		KeyStore keystore=KeyStore.getInstance(KeyStore.getDefaultType());
		FileInputStream f=new FileInputStream(keystoreFileName);
		try{
			keystore.load(f, pwd);
			java.security.cert.CertificateFactory cf=java.security.cert.CertificateFactory.getInstance("X.509");
			ByteArrayInputStream in=new ByteArrayInputStream(cert.derEncode());
			java.security.cert.Certificate jceCert=cf.generateCertificate(in);
			String alias=keystore.getCertificateAlias(jceCert);
			if(alias==null) {
				throw new Exception("cert not in keystore");
			}
			Key key=keystore.getKey(alias,pwd);
			if(key==null){
				throw new Exception("cert not private key in keystore");
			}
			return new JCESigner((PrivateKey)key);
		} finally{
			f.close();
		}
	}
	...
	private static AlgorithmIdentifier createSignAlgo(String algo) throws PkiException {
		if(algo.equalsIgnoreCase("SHA1WITHRSA")
			...
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifierNullParam(AlgorithmIdentifier.SM3WithSM2_OID);
				...
				|| algo.equalsIgnoreCase("SHA1WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA1_OID);
				...
				|| algo.equalsIgnoreCase("SHA224WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA224_OID);
				...
				|| algo.equalsIgnoreCase("SHA256WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA256_OID);
				...
				|| algo.equalsIgnoreCase("SHA384WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA384_OID);
				...
				|| algo.equalsIgnoreCase("SHA512WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA512_OID);
				...
				|| algo.equalsIgnoreCase("SHA3-224WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA3_224_OID);
				...
				|| algo.equalsIgnoreCase("SHA3-256WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA3_256_OID);
				...
				|| algo.equalsIgnoreCase("SHA3-384WITHECDSA")) {
			return 	AlgorithmIdentifier.CreateAlgorithmIdentifier(AlgorithmIdentifier.ECDSAWithSHA3_384_OID);
				...
				|| algo.equalsIgnoreCase("SHA3-512WITHECDSA")) {
