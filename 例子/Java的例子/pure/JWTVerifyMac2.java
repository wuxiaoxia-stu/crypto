...
public class JWTVerifyMac2 {

	public static void main(String[] args) throws Exception{
		if(args.length!=2) {
            System.out.println("Usage: java JWTVerifyMac2 hexKey jwt");
            return;
        }
		byte[] key=Hex.decode(args[0]);
		String jwtData=args[1];
		JWT jwt=JWT.decode(jwtData)
				.setMacImplement(new net.netca.pki.encoding.json.jose.impl.jce.JCEMac());
				
		if(jwt.verifyMac(key)==false){
