...
int main(int argc,char **argv)
{
	...
	rv=NetcaPKICryptoPublicKeyEncrypt(hPublicKey,NETCA_PKI_ALGORITHM_SM2_ENC,
	...
	rv=NetcaPKICryptoSM2CipherToDER(NETCA_PKI_ECC_CURVE_SM2,NETCA_PKI_ALGORITHM_SM3,
		out,outLen,&cipher,&cipherLen);
	NetcaPKICryptoFreeMemory(out);
	if(rv!=NETCA_PKI_SUCCESS)
	{
		printf("NetcaPKICryptoSM2CipherToDER fail\n");
		return -1;
	}
	rv=SaveFile(argv[3],cipher,cipherLen);
