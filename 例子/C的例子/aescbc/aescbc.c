...
int main(int argc,char **argv)
{
	...
	hCipher=NetcaPKICryptoGetCipherHandle(enc,NETCA_PKI_ALGORITHM_AES_CBC,
		key,keyLen,iv);
	NetcaPKICryptoFreeMemory(key);
	NetcaPKICryptoFreeMemory(iv);
	...
	rv=NetcaPKICryptoCipherSetPadding(hCipher,NETCA_PKI_PADDING_PKCS5);
	if(rv!=NETCA_PKI_SUCCESS)
	{
		NetcaPKICryptoFreeCipher(hCipher);
		printf("NetcaPKICryptoCipherSetPadding fail\n");
		return -1;
	}
