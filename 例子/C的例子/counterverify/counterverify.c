...
int main(int argc,char **argv)
{
	...
	for(i=0;i<count;i++)
	{
		rv=NetcaPKIMsgSignedDataGetIntegerAttribute(hSignedData,
			NETCA_PKI_SIGNEDDATA_ATTRIBUTE_COUNTERSIGNATURE_COUNT,i,&counterSignCount);
		if(rv==NETCA_PKI_SUCCESS)
		{
			for(j=0;j<counterSignCount;j++)
			{
				rv=NetcaPKIMsgSignedDataVerifyCountersignature(hSignedData,i,j,0);
				if(rv!=NETCA_PKI_SUCCESS)
				{
				...
				{
					cert=NetcaPKIMsgSignedDataGetCountersignatureCert(hSignedData,i,j);
