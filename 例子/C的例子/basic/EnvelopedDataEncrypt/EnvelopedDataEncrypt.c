...
int main(int argc,char **argv)
{
	...
	rv=NetcaPKIAppEnvelopedDataEncrypt(cert,(const unsigned char *)clearText,(int)strlen(clearText),
		&cipherText,&cipherTextLen,NULL);
	NetcaPKICertFreeCert(cert);
	if(rv!=NETCA_PKI_SUCCESS)
	{
		printf("NetcaPKIAppEnvelopedDataEncrypt fail,return %d\n",rv);
		return -1;
	}
