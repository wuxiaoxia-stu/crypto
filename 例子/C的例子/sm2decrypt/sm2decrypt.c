...
int main(int argc,char **argv)
{
	...
	rv= NetcaPKISelectCert("Decrypt","Algorithm='SM2' && CertType='Encrypt'",&hCert);
	...
	rv=NetcaPKICryptoSM2CipherFromDER(NETCA_PKI_ECC_CURVE_SM2,buf,bufLen,&cipher,&cipherLen);
	NetcaPKICryptoFreeMemory(buf);
	if(rv!=NETCA_PKI_SUCCESS)
	{
		NetcaPKICryptoFreeKeypair(hKeyPair);
		printf("NetcaPKICryptoSM2CipherFromDER fail\n");
		return -1;
	}

	rv=NetcaPKICryptoPrivateKeyDecrypt(hKeyPair,NETCA_PKI_ALGORITHM_SM2_ENC,
		0,cipher,cipherLen,&out,&outLen);
	NetcaPKICryptoFreeKeypair(hKeyPair);
	NetcaPKICryptoFreeMemory(cipher);
	if(rv!=NETCA_PKI_SUCCESS)
	{
