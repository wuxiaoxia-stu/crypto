...
function usage(scriptName)
    ...
    print(toANSIString("\t对inFileName文件进行DES ECB模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生DesEcbSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，一行有两列，第一列是加密或者解密时间，第二列是读文件时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s /e 1234567812345678 1.pdf 100",scriptName))
    print(toANSIString("\t使用密钥1234567812345678对1.pdf文件进行DES ECB加密100次"))
    print(string.format("\tlua %s /d 1234567812345678 1.enc 100",scriptName))
    print(toANSIString("\t使用密钥1234567812345678对1.enc文件进行DES ECB解密100次"))
    print(string.format("\tlua %s /e 1234567812345678 1.pdf 100 11 openssl",scriptName))
    print(toANSIString("\t使用openssl软设备对1.pdf文件进行DES ECB加密100次"))
end

function NewDesEcb(deviceObj,enc,key)
    if deviceObj then
        return deviceObj:NewECBCipher(enc,CryptCore.Const.DES_ECB,key,CryptCore.Const.PADDING_PKCS7)
    else
        return CryptCore.NewECBCipher(enc,CryptCore.Const.DES_ECB,key,CryptCore.Const.PADDING_PKCS7)
    end
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("DesEcbSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local devicePwd
local deviceObj=nil
local algo=CryptCore.Const.DES_ECB
local usage
if enc then
    usage="encrypt"
else
...
if argCount>4 then
    ...
    if not deviceObj:IsSupportedAlgorithm{["algo"]=algo,["usage"]=usage,["keybits"]=keyBits} then
        print(toANSIString(("设备不支持DES ECB算法")))
        os.exit(-1)
    end
end
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewDesEcb(deviceObj,enc,key))
    cipherTimer:Stop()
