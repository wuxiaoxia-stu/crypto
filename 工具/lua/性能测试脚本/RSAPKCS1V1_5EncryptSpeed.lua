...
function usage(scriptName)
    ...
    print(toANSIString("\t使用RSA加密证书里的公钥进行PKCS#1 V1.5填充的RSA加密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加密运算"))
    print(toANSIString("\t成功运行后产生RSAPKCS1V1_5EncryptSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，每一列是加密时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s rsa.cer 10 100",scriptName))
    print(toANSIString("\t使用rsa.cer的公钥进行PKCS#1 V1.5填充的RSA加密运算100次"))
    print(string.format("\tlua %s rsa.cer 10 100 11 openssl",scriptName))
    print(toANSIString("\t使用openssl软设备导入rsa.cer的公钥进行PKCS#1 V1.5填充的RSA加密运算100次"))
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("RSAPKCS1V1_5EncryptSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
if math.floor((keyBits+7)/8) <inLen+11 then
    print(toANSIString("明文长度太长"));
    os.exit(-1)
end
...
for i=1,count do
    ...
    cipherTimer:Start()
    data=assert(publicKey:Encrypt { ["algo"]=CryptCore.Const.RSA_PKCS1_V1_5_ENC,["clear"]=inData })
    cipherTimer:Stop()
...
print(toANSIString(string.format("总运行时间:%f毫秒,速度:%.3fMB/s",time,inLen*count/1024/1024/time*1000)))
if connectTime then
