...
function usage(scriptName)
    ...
    print(toANSIString("\t使用RSA加密证书对应的私钥进行不填充的RSA解密运算，重复执行count次。"))
    print(toANSIString("\t成功运行后产生RSADecryptSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，每一列是解密时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s rsa.cer 100 12345678",scriptName))
    print(toANSIString("\t使用rsa.cer的私钥进行不填充的RSA解密运算100次"))
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("RSADecryptSpeed_%04d%02d%02d%02d%02d%02d.csv",
