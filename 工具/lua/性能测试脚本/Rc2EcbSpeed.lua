...
function usage(scriptName)
    print(string.format("lua %s /e|/d key effectiveBits inFileName count [deviceType deviceSn [pwd]]",scriptName))
    ...
    print(toANSIString("\t对inFileName文件进行RC2 ECB模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生Rc2EcbSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，一行有两列，第一列是加密或者解密时间，第二列是读文件时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s /e 12345678123456781234567812345678 128 1.pdf 100",scriptName))
    print(toANSIString("\t使用密钥12345678123456781234567812345678对1.pdf文件进行RC2 ECB加密100次"))
    print(string.format("\tlua %s /d 12345678123456781234567812345678 128 1.enc 100",scriptName))
    print(toANSIString("\t使用密钥12345678123456781234567812345678对1.enc文件进行RC2 ECB解密100次"))
    print(string.format("\tlua %s /e 12345678123456781234567812345678 128 1.pdf 100 11 openssl",scriptName))
    print(toANSIString("\t使用openssl软设备对1.pdf文件进行RC2 ECB加密100次"))
end

function NewRc2Ecb(deviceObj,enc,key,bits)
    if deviceObj then
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("Rc2EcbSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
if argCount>5 then
    ...
    if not deviceObj:IsSupportedAlgorithm{["algo"]=algo,["usage"]=usage,["keybits"]=keyBits,["effectivebits"]=effectiveBits} then
        print(toANSIString(("设备不支持RC2 ECB算法")))
        os.exit(-1)
    end
end
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewRc2Ecb(deviceObj,enc,key,effectiveBits))
    cipherTimer:Stop()
