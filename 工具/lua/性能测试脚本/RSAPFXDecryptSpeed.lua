...
function usage(scriptName)
    ...
    print(toANSIString("\t使用PFX文件里的第一张证书对应的私钥进行不填充的RSA解密运算，重复执行count次。"))
    print(toANSIString("\t成功运行后产生RSAPFXDecryptSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，每一列是解密时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s rsa.pfx 100 12345678",scriptName))
    print(toANSIString("\t使用rsa.pfx的第一张证书对应的私钥进行不填充的RSA解密运算100次"))
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("RSAPFXDecryptSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local clearData= deviceObj:GenerateRandom(keyLen-1)
deviceObj:Free()
deviceObj=nil
...
clearData="\0" ..clearData

local cipherData=assert(publicKey:Encrypt { ["algo"]=CryptCore.Const.RSA_RAW_ENC,["clear"]=clearData })
publicKey:Free()
...
for i=1,count do
    ...
    cipherTimer:Start()
    data=assert(keypairObj:Decrypt { ["algo"]=CryptCore.Const.RSA_RAW_ENC,["cipher"]=cipherData })
    cipherTimer:Stop()
