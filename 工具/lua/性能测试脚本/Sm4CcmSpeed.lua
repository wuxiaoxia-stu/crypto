...
function usage(scriptName)
    ...
    print(toANSIString("\t对inFileName文件进行SM4 CCM模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生Sm4CcmSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的加密或者解密时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s /e 12345678123456781234567890ABCDEF  1234567812345678123456 \"\" 16 1.pdf 100",scriptName))
    print(toANSIString("\t对1.pdf文件进行SM4 CCM加密100次"))
    print(string.format("\tlua %s /d 12345678123456781234567890ABCDEF 1234567812345678123456 \"\" 16 1.enc 100",scriptName))
    print(toANSIString("\t对1.enc文件进行SM4 CCM解密100次"))
    
end

function NewSm4Ccm(deviceObj,enc,key,nonce,tLen,aad,p)
    if deviceObj then
        return deviceObj:NewCCMCipher(enc,CryptCore.Const.SM4_CCM,key,tLen,nonce,aad,p)
    else
        return CryptCore.NewCCMCipher(enc,CryptCore.Const.SM4_CCM,key,tLen,nonce,aad,p)
    end
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("Sm4CcmSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local devicePwd
local deviceObj=nil
local algo=CryptCore.Const.SM4_CCM
local usage
if enc then
    usage="encrypt"
else
...
if argCount>7 then
    ...
    if not deviceObj:IsSupportedAlgorithm{["algo"]=algo,["usage"]=usage,["keybits"]=keyBits,["tlen"]=tLen,["nonce"]=nonce,["adata"]=adata,["p"]=fileLength} then
        print(toANSIString("设备不支持SM4 CCM算法"))
        os.exit(-1)
    end
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewSm4Ccm(deviceObj,enc,key,nonce,tLen,adata,fileLength))
