...
function usage(scriptName)
    ...
    print(toANSIString("\t对inFileName文件进行SM4 CTR模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生Sm4CtrSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，一行有两列，第一列是加密或者解密时间，第二列是读文件时间"))
    ...
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.pdf文件进行SM4 CTR加密100次"))
    print(string.format("\tlua %s /d 12345678123456781234567890ABCDEF 12345678123456781234567812345678 128 1.enc 100",scriptName))
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.enc文件进行SM4 CTR解密100次"))
    
end

function NewSm4Ctr(deviceObj,enc,key,iv,bits)
    if deviceObj then
        return deviceObj:NewCTRCipher(enc,CryptCore.Const.SM4_CTR,key,iv,bits)
    else
        return CryptCore.NewCTRCipher(enc,CryptCore.Const.SM4_CTR,key,iv,bits)
    end
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("Sm4CtrSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local keyBits=#key*8
if keyBits~=128 then
    print(toANSIString("SM4密钥必须是128位的"));
    os.exit(-1)
end
...
local devicePwd
local deviceObj=nil
local algo=CryptCore.Const.SM4_CTR
local usage
if enc then
    usage="encrypt"
else
...
if argCount>6 then
    ...
    if not deviceObj:IsSupportedAlgorithm{["algo"]=algo,["usage"]=usage,["keybits"]=keyBits,["countbits"]=countBits} then
        print(toANSIString(string.format("设备不支持计数器位数为%d位的SM4 CTR算法",countBits)))
        os.exit(-1)
    end
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewSm4Ctr(deviceObj,enc,key,iv,countBits))
    cipherTimer:Stop()
