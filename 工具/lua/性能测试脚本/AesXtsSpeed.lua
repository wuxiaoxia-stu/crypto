...
function usage(scriptName)
    print(string.format("lua %s /e|/d key tweak inFileName count [deviceType deviceSn [pwd]]",scriptName))
    ...
    print(toANSIString("\t对inFileName文件进行AES XTS模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生AesXtsSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的加密或者解密时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s /e 12345678123456781234567890ABCDEF1234567890ABCDEF1234567890ABCDEF  12345678123456781234567812345678 1.pdf 100",scriptName))
    print(toANSIString("\t对1.pdf文件进行AES XTS加密100次"))
    print(string.format("\tlua %s /d 12345678123456781234567890ABCDEF 12345678123456781234567812345678  1.enc 100",scriptName))
    print(toANSIString("\t对1.enc文件进行AES XTS解密100次"))
    ...
    print(toANSIString("\t使用openssl软设备对1.pdf文件进行AES XTS加密100次"))
end

function NewAesXts(deviceObj,enc,key,tweak)
    if deviceObj then
        return deviceObj:NewXTSCipher(enc,CryptCore.Const.AES_XTS,key,tweak)
    else
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("AesXtsSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local devicePwd
local deviceObj=nil
local algo=CryptCore.Const.AES_XTS
local usage
if enc then
    usage="encrypt"
else
...
if argCount>5 then
    ...
    if not deviceObj:IsSupportedAlgorithm{["algo"]=algo,["usage"]=usage,["keybits"]=keyBits,["tweak"]=tweak} then
        print(toANSIString("设备不支持AES XTS算法"))
        os.exit(-1)
    end
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewAesXts(deviceObj,enc,key,tweak))
