...
function usage(scriptName)
    print(string.format("lua %s pfxFileName clearLen count [pwd]",scriptName))
    print(toANSIString("参数"))
    print(toANSIString("\tpfxFileName：PFX文件名"))
    print(toANSIString("\tclearLen：明文长度，字节数"))
    print(toANSIString("\tcount：重复执行的次数"))
    print(toANSIString("\tpwd：使用PFX的密码"))
    print(toANSIString("说明"))
    print(toANSIString("\t使用PFX文件里的第一张证书对应的私钥进行SM2解密运算，重复执行count次。"))
    print(toANSIString("\t成功运行后产生SM2PFXDecryptSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，每一列是解密时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s sm2.pfx 32 100 12345678",scriptName))
    print(toANSIString("\t使用sm2.pfx的第一张证书对应的私钥进行SM2解密运算100次"))
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("SM2DecryptSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local argCount=#arg
if argCount~=4 and argCount~=3 then
    usage(arg[0])
    os.exit(-1)
end
...
local clearData= deviceObj:GenerateRandom(clearLen)
deviceObj:Free()
deviceObj=nil
...
local cipherData=assert(publicKey:Encrypt { ["algo"]=CryptCore.Const.SM2_ENC,["clear"]=clearData })
publicKey:Free()
...
for i=1,count do
    ...
    cipherTimer:Start()
    data=assert(keypairObj:Decrypt { ["algo"]=CryptCore.Const.SM2_ENC,["cipher"]=cipherData })
    cipherTimer:Stop()
...
print(toANSIString(string.format("解密最大时间:%.3f毫秒,解密最小时间:%.3f毫秒,解密平均时间:%.3f毫秒,解密平均速度:%.1fKB/s,每秒解密%.1f次",maxTime,minTime,avgTime,cipherLen/1024/avgTime*1000,1000/avgTime)))
