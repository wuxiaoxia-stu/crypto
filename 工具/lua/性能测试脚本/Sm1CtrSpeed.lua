...
function usage(scriptName)
    ...
    print(toANSIString("\t对inFileName文件进行SM1 CTR模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生Sm1CtrSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，一行有两列，第一列是加密或者解密时间，第二列是读文件时间"))
    ...
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.pdf文件进行SM1 CTR加密100次"))
    print(string.format("\tlua %s /d 12345678123456781234567890ABCDEF 12345678123456781234567812345678 128 1.enc 100",scriptName))
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.enc文件进行SM1 CTR解密100次"))
...
function NewSm1Ctr(deviceObj,enc,key,iv,bits)
    if deviceObj then
        ...
        return CryptCore.NewCTRCipher(enc,CryptCore.Const.SM1_CTR,key,iv,bits)
    end
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("Sm1CtrSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewSm1Ctr(deviceObj,enc,key,iv,countBits))
    cipherTimer:Stop()
