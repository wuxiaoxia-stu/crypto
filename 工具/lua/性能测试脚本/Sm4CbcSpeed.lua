...
function usage(scriptName)
    ...
    print(toANSIString("\t对inFileName文件进行SM4 CBC模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生Sm4CbcSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，一行有两列，第一列是加密或者解密时间，第二列是读文件时间"))
    ...
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.pdf文件进行SM4 CBC加密100次"))
    print(string.format("\tlua %s /d 12345678123456781234567890ABCDEF 12345678123456781234567812345678 1.enc 100",scriptName))
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.enc文件进行SM4 CBC解密100次"))
    
end

function NewSm4Cbc(deviceObj,enc,key,iv)
    if deviceObj then
        return deviceObj:NewCBCCipher(enc,CryptCore.Const.SM4_CBC,key,iv,CryptCore.Const.PADDING_PKCS7)
    else
        return CryptCore.NewCBCCipher(enc,CryptCore.Const.SM4_CBC,key,iv,CryptCore.Const.PADDING_PKCS7)
    end
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("Sm4CbcSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local devicePwd
local deviceObj=nil
local algo=CryptCore.Const.SM4_CBC
local usage
if enc then
    usage="encrypt"
else
...
if argCount>5 then
    ...
    if not deviceObj:IsSupportedAlgorithm{["algo"]=algo,["usage"]=usage,["keybits"]=keyBits,["iv"]=iv} then
        print(toANSIString("设备不支持SM4 CBC算法",keyBits))
        os.exit(-1)
    end
end
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewSm4Cbc(deviceObj,enc,key,iv))
    cipherTimer:Stop()
