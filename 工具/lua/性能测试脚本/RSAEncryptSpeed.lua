...
function usage(scriptName)
    print(string.format("lua %s certFileName count [deviceType deviceSn [pwd]]",scriptName))
    ...
    print(toANSIString("\t使用RSA加密证书里的公钥进行不填充的RSA加密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加密运算"))
    print(toANSIString("\t成功运行后产生RSAEncryptSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，每一列是加密时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s rsa.cer 100",scriptName))
    print(toANSIString("\t使用rsa.cer的公钥进行不填充的RSA加密运算100次"))
    print(string.format("\tlua %s rsa.cer 100 11 openssl",scriptName))
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("RSAEncryptSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local inData="\0" .. deviceObj:GenerateRandom(keyLen-1)
deviceObj:Free()
deviceObj=nil
if inData==nil then
    publicKey:Free()
...
for i=1,count do
    ...
    cipherTimer:Start()
    data=assert(publicKey:Encrypt { ["algo"]=CryptCore.Const.RSA_RAW_ENC,["clear"]=inData })
    cipherTimer:Stop()
...
print(toANSIString(string.format("加密最大时间:%.3f毫秒,加密最小时间:%.3f毫秒,加密平均时间:%.3f毫秒,加密平均速度:%.1fMB/s,每秒加密%.1f次",maxTime,minTime,avgTime,keyLen/1024/1024/avgTime*1000,1000/avgTime)))
