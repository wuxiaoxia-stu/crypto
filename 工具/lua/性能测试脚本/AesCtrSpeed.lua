...
function usage(scriptName)
    print(string.format("lua %s /e|/d key iv bits inFileName count [deviceType deviceSn [pwd]]",scriptName))
    ...
    print(toANSIString("\t对inFileName文件进行AES CTR模式的加密或者解密运算，重复执行count次。如果指定设备类型和序列号，则使用指定的设备进行加解密运算"))
    print(toANSIString("\t成功运行后产生AesCtrSpeed_YYYYMMDDhhmmss.csv文件，其中每行表示一次的运行结果，一行有两列，第一列是加密或者解密时间，第二列是读文件时间"))
    print(toANSIString("例子"))
    print(string.format("\tlua %s /e 12345678123456781234567890ABCDEF  12345678123456781234567812345678 128 1.pdf 100",scriptName))
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.pdf文件进行AES CTR加密100次"))
    print(string.format("\tlua %s /d 12345678123456781234567890ABCDEF 12345678123456781234567812345678 128 1.enc 100",scriptName))
    print(toANSIString("\t使用密钥12345678123456781234567890ABCDEF，IV 12345678123456781234567812345678对1.enc文件进行AES CTR解密100次"))
    print(string.format("\tlua %s /e 12345678123456781234567890ABCDEF 12345678123456781234567812345678 128 1.pdf 100 11 openssl",scriptName))
    print(toANSIString("\t使用openssl软设备对1.pdf文件进行AES CTR加密100次"))
end

function NewAesCtr(deviceObj,enc,key,iv,bits)
    if deviceObj then
        return deviceObj:NewCTRCipher(enc,CryptCore.Const.AES_CTR,key,iv,bits)
    else
        return CryptCore.NewCTRCipher(enc,CryptCore.Const.AES_CTR,key,iv,bits)
    end
end
...
function GetCsvFileName()
    local t=os.date("*t")
    return string.format("AesCtrSpeed_%04d%02d%02d%02d%02d%02d.csv",
...
local argCount=#arg
if argCount~=6 and argCount~=8 and argCount~=9 then
    usage(arg[0])
    os.exit(-1)
end
...
local devicePwd
local deviceObj=nil
local algo=CryptCore.Const.AES_CTR
local usage
if enc then
    usage="encrypt"
else
...
if argCount>6 then
    ...
    if not deviceObj:IsSupportedAlgorithm{["algo"]=algo,["usage"]=usage,["keybits"]=keyBits,["countbits"]=countBits} then
        print(toANSIString(string.format("设备不支持计数器位数为%d位的AES-%d CTR算法",countBits,keyBits)))
        os.exit(-1)
    end
...
for i=1,count do
    ...
    cipherTimer:Start()
    cipherObj=assert(NewAesCtr(deviceObj,enc,key,iv,countBits))
    cipherTimer:Stop()
